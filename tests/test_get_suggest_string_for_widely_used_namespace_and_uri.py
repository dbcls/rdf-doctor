...
from doctor.doctor import get_widely_used_prefixes_dict, get_suggest_string_for_widely_used_namespace_and_uri
...
class TestGetSuggestStringForWidelyUsedNamespaceAndUri(unittest.TestCase):
    ...
    def test_chebi(self):
        ...
        suggest_string = get_suggest_string_for_widely_used_namespace_and_uri("oboc:", "http://www.ebi.ac.uk/chebi/searchId.do?chebiId=CHEBI%3A", widely_used_prefixes_dict, str(Path(__file__).resolve().parent.joinpath(REFINE_PREFIX_URIS_FILE_PATH)))
        print(suggest_string)
