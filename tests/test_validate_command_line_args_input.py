...
class TestValidateCommnadLineArgsInput(unittest.TestCase):
    ...
    def test_input_nt(self):
        error_msg = validate_command_line_args_input([NT_1])
        self.assertEqual(error_msg, None)

    # [input/OK] n-triple multi
    def test_input_nt_multi(self):
        error_msg = validate_command_line_args_input([NT_1, NT_2, NT_3])
        self.assertEqual(error_msg, None)

    # [input/OK] turtle
    def test_input_ttl(self):
        error_msg = validate_command_line_args_input([TTL_1])
        self.assertEqual(error_msg, None)

    # [input/OK] turtle multi
    def test_input_ttl_multi(self):
        error_msg = validate_command_line_args_input([TTL_1, TTL_2, TTL_3])
        self.assertEqual(error_msg, None)

    # [input/OK] n-triple gz
    def test_input_nt_gz(self):
        error_msg = validate_command_line_args_input([NT_1_GZ])
        self.assertEqual(error_msg, None)

    # [input/OK] n-triple gz multi
    def test_input_nt_gz_multi(self):
        error_msg = validate_command_line_args_input([NT_1_GZ, NT_2_GZ, NT_3_GZ])
        self.assertEqual(error_msg, None)
    ...
    def test_input_ttl_gz(self):
        error_msg = validate_command_line_args_input([TTL_1_GZ])
        self.assertEqual(error_msg, None)

    # [input/OK] turtle gz multi
    def test_input_ttl_gz_multi(self):
        error_msg = validate_command_line_args_input([TTL_1_GZ, TTL_2_GZ, TTL_3_GZ])
        self.assertEqual(error_msg, None)
    ...
    def test_input_nonexistent_file(self):
        error_msg = validate_command_line_args_input([BASE_DIR + "aaa.txt"])
    ...
    def test_input_unsupported_extension(self):
        error_msg = validate_command_line_args_input([str(Path(__file__).resolve().parent.parent.joinpath("doctor").joinpath(PREFIXES_FILE_PATH))])
    ...
    def test_input_different_extension(self):
        error_msg = validate_command_line_args_input([TTL_1, NT_1])
